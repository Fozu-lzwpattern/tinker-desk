...
import type { TinkerMessage, AgentInfo, AgentStatus, AgentProfile, ReputationScore, CapabilityChannel, Intent, IntentStatus, TrendingRoom, BanRecord, RoomRules, GovernanceModel, RuleProposal } from './types.js'
...
export const db: BetterSqliteDB = new Database(DB_FILE)
...
export function persistMessage(msg: TinkerMessage): void {
...
/** 查询 room 历史消息（从旧到新） */
export function getRoomHistory(roomId: string, limit = 50): TinkerMessage[] {
...
export function getDmHistory(nodeIdA: string, nodeIdB: string, limit = 50): TinkerMessage[] {
  const rows = db.prepare(`
    SELECT * FROM messages
    WHERE room IS NULL
      AND (
    ...
    ORDER BY timestamp ASC
    LIMIT ?
  `).all(nodeIdA, nodeIdB, nodeIdB, nodeIdA, limit) as MessageRow[]
  return rows.map(rowToMessage)
}

/** 查询某 Agent 的最近消息（发送 + 接收） */
export function getAgentHistory(nodeId: string, limit = 50): TinkerMessage[] {
  const rows = db.prepare(`
    SELECT * FROM messages
    WHERE from_id = ? OR to_id = ?
    ORDER BY timestamp DESC
    LIMIT ?
  `).all(nodeId, nodeId, limit) as MessageRow[]
...
const stmtFindAgents = (role?: string, capability?: string): AgentRow[] => {
...
export function upsertAgent(info: {
  nodeId: string; publicKey: string; displayName: string
  role?: string; capabilities?: string[]; online?: boolean
}): void {
  stmtUpsertAgent.run({
    nodeId:       info.nodeId,
...
/** 设置 Agent 在线/离线 */
export function dbSetOnline(nodeId: string, online: boolean): void {
...
export function recordEdge(fromId: string, toId: string): void {
  const ts = Date.now()
...
export function getGraphData(): GraphData {
  const nodes = (stmtGetAllAgents.all() as AgentRow[]).map(rowToAgentInfo)
  const edgeRows = db.prepare(`
    SELECT source_id, target_id, weight, last_active
    FROM edges
    ORDER BY weight DESC
  `).all() as { source_id: string; target_id: string; weight: number; last_active: number }[]
...
export function getAgentNeighbors(nodeId: string): { neighbor: AgentInfo; weight: number }[] {
  const rows = db.prepare(`
  ...
  `).all(nodeId) as (AgentRow & { weight: number })[]
...
/** 获取单个 Agent */
export function dbGetAgent(nodeId: string): AgentInfo | null {
  const row = stmtGetAgent.get(nodeId) as AgentRow | undefined
  return row ? rowToAgentInfo(row) : null
}

/** 获取所有 Agent */
export function dbGetAllAgents(): AgentInfo[] {
  return (stmtGetAllAgents.all() as AgentRow[]).map(rowToAgentInfo)
}

/** 按条件筛选 Agent */
export function dbFindAgents(filter: { role?: string; capability?: string }): AgentInfo[] {
  return stmtFindAgents(filter.role, filter.capability).map(rowToAgentInfo)
}

/** Relay 启动时加载所有 Agent 到内存注册表 */
export function loadPersistedAgents(): AgentInfo[] {
  const rows = stmtGetAllAgents.all() as AgentRow[]
  // 重启后所有 Agent 标记为离线
...
export function getProfiles(nodeId: string, facetName?: string): AgentProfile[] {
  const rows = facetName
    ? [stmtGetProfile.get(nodeId, facetName)].filter(Boolean) as ProfileRow[]
    : stmtGetProfiles.all(nodeId) as ProfileRow[]
...
export function addEndorsement(
  nodeId: string,
  facetName: string,
  endorsement: { fromNodeId: string; capability: string; comment?: string; timestamp: number; signature: string }
): void {
  const row = stmtGetProfileEndorsements.get(nodeId, facetName) as { endorsements: string | null } | undefined
  if (!row) return  // profile doesn't exist
  const existing: AgentProfile['endorsements'] = row.endorsements ? JSON.parse(row.endorsements) : []
...
export function updateObserved(
  nodeId: string,
  facetName: string,
  observed: AgentProfile['observed']
): void {
...
export function getReputation(nodeId: string): ReputationScore | null {
  const row = stmtGetReputation.get(nodeId) as ReputationRow | undefined
  return row ? rowToReputation(row) : null
}

export function upsertReputation(rep: ReputationScore): void {
...
export function getAllReputations(minTrust?: number): ReputationScore[] {
  const rows = stmtGetAllReputations.all() as ReputationRow[]
  const scores = rows.map(rowToReputation)
  if (minTrust !== undefined) return scores.filter(s => s.trustScore >= minTrust)
  return scores
}
 ...
 */
export function recalculateReputation(nodeId: string): ReputationScore {
  const now = Date.now()
  ...
  `).get(nodeId, sevenDaysAgo) as { c: number }
  ...
  const profileRows = stmtGetProfiles.all(nodeId) as ProfileRow[]
  let endorsementCount = 0
  let endorsementSum = 0
  for (const row of profileRows) {
    const endorsements: AgentProfile['endorsements'] = row.endorsements ? JSON.parse(row.endorsements) : []
  ...
  `).get(nodeId) as { last: number | null }
...
export function upsertCapabilityChannel(channel: CapabilityChannel): void {
...
export function getCapabilityChannels(): CapabilityChannel[] {
  const rows = stmtGetAllCapabilityChannels.all() as CapabilityChannelRow[]
  return rows.map(rowToCapabilityChannel)
}

export function getCapabilityChannel(capability: string): CapabilityChannel | null {
  const row = stmtGetCapabilityChannel.get(capability) as CapabilityChannelRow | undefined
...
interface MessageRow {
  id: string; type: string; from_id: string; to_id: string | null
  room: string | null; content: string; attachment: string | null
  timestamp: number; signature: string
}

interface AgentRow {
  node_id: string; public_key: string; display_name: string
  role: string | null; capabilities: string | null
  online: number; registered_at: number; last_seen_at: number
  last_heartbeat_at: number; status: string
}

function rowToMessage(row: MessageRow): TinkerMessage {
  return {
    id:        row.id,
    type:      row.type as TinkerMessage['type'],
    from:      row.from_id,
...
function rowToAgentInfo(row: AgentRow): AgentInfo {
  const status = (row.status || (row.online === 1 ? 'active' : 'offline')) as AgentStatus
  return {
    nodeId:       row.node_id,
...
interface CapabilityChannelRow {
  room_id:     string
  capability:  string
...
function rowToReputation(row: ReputationRow): ReputationScore {
  return {
    nodeId:            row.node_id,
...
function rowToCapabilityChannel(row: CapabilityChannelRow): CapabilityChannel {
  return {
    roomId:      row.room_id,
...
export function dbUpdateHeartbeat(nodeId: string): void {
  const now = Date.now()
  stmtUpdateHeartbeat.run(now, now, nodeId)
}

/** 设置 Agent 状态 */
export function dbSetAgentStatus(nodeId: string, status: AgentStatus): void {
  stmtSetAgentStatus.run(status, status !== 'offline' ? 1 : 0, nodeId)
}

/** 获取应标记为 stale 的 Agent（心跳超时） */
export function dbGetStaleAgents(thresholdMs: number): AgentInfo[] {
  const cutoff = Date.now() - thresholdMs
  return (stmtGetStaleAgents.all(cutoff) as AgentRow[]).map(rowToAgentInfo)
}

/** 获取 Agent 健康状态摘要 */
export function dbGetAgentHealthSummary(): { active: number; stale: number; offline: number } {
  const active  = (db.prepare(`SELECT COUNT(*) as c FROM agents WHERE status = 'active'`).get() as { c: number }).c
  const stale   = (db.prepare(`SELECT COUNT(*) as c FROM agents WHERE status = 'stale'`).get() as { c: number }).c
  const offline = (db.prepare(`SELECT COUNT(*) as c FROM agents WHERE status = 'offline'`).get() as { c: number }).c
...
interface IntentRow {
  intent_id: string; from_node_id: string; capability: string
  description: string; constraints: string | null; ttl_ms: number
  status: string; created_at: number; matched_by: string | null; matched_at: number | null
}
...
export function dbCreateIntent(intent: Omit<Intent, 'status' | 'matchedBy' | 'matchedAt'>): void {
...
export function dbGetIntent(intentId: string): Intent | null {
  const row = stmtGetIntent.get(intentId) as IntentRow | undefined
  return row ? rowToIntent(row) : null
}

export function dbGetOpenIntents(): Intent[] {
  return (stmtGetOpenIntents.all() as IntentRow[]).map(rowToIntent)
}

export function dbMatchIntent(intentId: string, matchedNodeId: string): Intent | null {
  const now = Date.now()
...
export function dbExpireIntents(): Intent[] {
  const now = Date.now()
  const rows = stmtExpireIntents.all(now) as IntentRow[]
...
interface RoomActivityRow {
  room_id: string; window_start: number; message_count: number; unique_senders: string
}

/** 记录一条消息到 room 的活跃度窗口 */
export function dbTrackRoomActivity(roomId: string, senderNodeId: string): void {
  const windowStart = Math.floor(Date.now() / WINDOW_SIZE_MS) * WINDOW_SIZE_MS

  // 获取当前窗口的 unique senders
  const existing = stmtGetRoomActivityWindow.get(roomId, windowStart) as RoomActivityRow | undefined
  let senders: string[]
  if (existing) {
    senders = JSON.parse(existing.unique_senders)
    if (!senders.includes(senderNodeId)) senders.push(senderNodeId)
  } else {
...
export function dbGetTrendingRooms(threshold = 20): TrendingRoom[] {
  const windowCutoff = Date.now() - WINDOW_SIZE_MS
  const rows = stmtGetTrendingRooms.all(windowCutoff, threshold) as { room_id: string; total_msgs: number; window_start: number }[]

  return rows.map(r => {
    // 获取该 room 在当前窗口的 unique senders
    const windowStart = Math.floor(Date.now() / WINDOW_SIZE_MS) * WINDOW_SIZE_MS
    const actRow = stmtGetRoomActivityWindow.get(r.room_id, windowStart) as RoomActivityRow | undefined
    const senders = actRow ? JSON.parse(actRow.unique_senders) as string[] : []
    return {
      roomId:        r.room_id,
...
export function dbCleanOldActivity(): void {
...
export function dbBanAgent(nodeId: string, bannedBy: string, reason: string, expiresAt?: number): BanRecord {
  const now = Date.now()
...
export function dbUnbanAgent(nodeId: string): void {
  stmtDeleteBan.run(nodeId)
}

export function dbGetBan(nodeId: string): BanRecord | undefined {
  const row = stmtGetBan.get(nodeId) as { node_id: string; banned_by: string; reason: string; banned_at: number; expires_at: number | null } | undefined
...
export function dbIsBanned(nodeId: string): boolean {
...
export function dbGetAllBans(): BanRecord[] {
  const rows = stmtGetAllBans.all() as { node_id: string; banned_by: string; reason: string; banned_at: number; expires_at: number | null }[]
...
export function dbCleanExpiredBans(): number {
...
interface RoomRulesRow {
  room_id: string; creator: string; min_trust: number; allowed_roles: string | null
  invite_only: number; allowed_message_types: string | null; max_message_length: number | null
  rate_limit: number | null; governance_model: string; rule_change_threshold: number | null; updated_at: number
}

function rowToRoomRules(row: RoomRulesRow): RoomRules {
  return {
    roomId:              row.room_id,
    creator:             row.creator,
    ...
    governanceModel:     row.governance_model as GovernanceModel,
    ruleChangeThreshold: row.rule_change_threshold ?? undefined,
    updatedAt:           row.updated_at,
  }
}

export function dbSetRoomRules(rules: RoomRules): void {
...
export function dbGetRoomRules(roomId: string): RoomRules | undefined {
  const row = stmtGetRoomRules.get(roomId) as RoomRulesRow | undefined
  return row ? rowToRoomRules(row) : undefined
}

export function dbDeleteRoomRules(roomId: string): void {
 ...
 */
export function dbCheckAccess(
  nodeId: string,
  action: 'broadcast' | 'proposal' | 'dm',
  roomId?: string,
  ...
  const repRow = db.prepare(`SELECT trust_score, total_interactions FROM reputation WHERE node_id = ?`)
    .get(nodeId) as { trust_score: number; total_interactions: number } | undefined
  ...
  if (roomId) {
    ...
    if (rules) {
      ...
      if (rules.allowedRoles) {
        const agentRow = db.prepare(`SELECT role FROM agents WHERE node_id = ?`).get(nodeId) as { role: string | null } | undefined
 ...
 */
export function dbCheckRateLimit(nodeId: string, roomId: string): boolean {
  const rules = dbGetRoomRules(roomId)
  if (!rules?.rateLimit) return true  // No limit
  const oneMinAgo = Date.now() - 60_000
  const row = db.prepare(`
    SELECT COUNT(*) as cnt FROM messages
    WHERE from_id = ? AND room = ? AND timestamp > ?
  `).get(nodeId, roomId, oneMinAgo) as { cnt: number }
...
interface ProposalRow {
  proposal_id: string; room_id: string; proposed_by: string; changes: string
  status: string; votes_for: number; votes_against: number; created_at: number; expires_at: number
}

function rowToProposal(row: ProposalRow): RuleProposal {
  return {
...
export function dbCreateProposal(
  proposalId: string, roomId: string, proposedBy: string,
...
export function dbGetProposal(proposalId: string): RuleProposal | undefined {
  const row = stmtGetProposal.get(proposalId) as ProposalRow | undefined
  return row ? rowToProposal(row) : undefined
}

export function dbGetRoomProposals(roomId: string): RuleProposal[] {
  const rows = stmtGetRoomProposals.all(roomId) as ProposalRow[]
 ...
 */
export function dbVoteOnProposal(
  proposalId: string, voter: string, vote: 'for' | 'against', weight = 1,
  ...
  // Re-tally
  const votes = stmtGetProposalVotes.all(proposalId) as { voter: string; vote: string; weight: number }[]
 ...
 */
export function dbEvaluateProposal(proposalId: string): 'passed' | 'rejected' | null {
  const proposal = dbGetProposal(proposalId)
  ...
  let result: 'passed' | 'rejected' | null = null
  if (forRatio >= threshold) result = 'passed'
  else if ((1 - forRatio) >= threshold) result = 'rejected'
  ...
  if (result) {
    ...
    if (result === 'passed' && rules) {
      const updated: RoomRules = { ...rules, ...proposal.changes, updatedAt: Date.now() }
...
export function dbExpireProposals(): number {
