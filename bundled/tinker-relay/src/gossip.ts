...
function log(...args: unknown[]): void {
  if (VERBOSE) console.log('[gossip]', ...args)
}
...
function connectPeer(address: string): void {
  if (peers.get(address)?.dead) return
...
export function addPeer(address: string): void {
...
export function getPeerCount(): number {
  ...
  for (const c of peers.values()) {
    if (c.ws?.readyState === WebSocket.WebSocket.OPEN) count++
  }
  return count
}
 ...
 */
export function gossipBroadcast(payload: unknown): void {
  ...
  if (sent > 0) log(`📡 gossip → ${sent} peer(s)`)
}

/**
 * 将 DM 消息转发给所有在线 peer relay。
 */
export function gossipDm(payload: unknown): void {
...
/** 优雅关闭所有 peer 连接 */
export function closeGossip(): void {
  for (const conn of peers.values()) {
    conn.dead = true
    conn.ws?.close()
  }
