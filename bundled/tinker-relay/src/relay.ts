...
import {
  ...
  broadcast, sendToNode, sendDirectToNode, broadcastGlobal,
  getRoomCount, getTotalConnections, getRoomList, getPublicKey,
  registerConnection, unregisterConnection, setPublicKey,
} from './rooms.js'
import {
  register, findAgents, agentCount,
  getAgentInfo, setOnline, setOffline, onlineCount,
} from './registry.js'
import {
  db,
  persistMessage, getRoomHistory, getDmHistory, getAgentHistory,
  upsertAgent, dbSetOnline, getGraphData, getAgentNeighbors,
  upsertProfile, getProfiles, addEndorsement,
  getReputation, recalculateReputation, getAllReputations,
  upsertCapabilityChannel, getCapabilityChannels, getCapabilityChannel,
  dbUpdateHeartbeat, dbSetAgentStatus, dbGetStaleAgents, dbGetAgentHealthSummary,
  dbCreateIntent, dbGetIntent, dbGetOpenIntents, dbMatchIntent, dbExpireIntents,
  dbTrackRoomActivity, dbGetTrendingRooms, dbCleanOldActivity,
  dbBanAgent, dbUnbanAgent, dbIsBanned, dbGetAllBans, dbCleanExpiredBans,
  dbSetRoomRules, dbGetRoomRules,
  dbCheckAccess, dbCheckRateLimit,
  dbCreateProposal, dbGetProposal, dbGetRoomProposals, dbVoteOnProposal, dbEvaluateProposal, dbExpireProposals,
  loadPersistedAgents, recordEdge,
} from './db.js'
import type {
  SignalingMsg, TinkerMessage, RegisterRequest, RegisterResponse,
  AgentInfo, RelayInfo, AgentProfile, CapabilityChannel, Intent,
  RoomRules, RuleProposal,
} from './types.js'
...
export class TinkerRelay {
  ...
  private readonly msgAuth: boolean
  ...
  private wsToNodeId = new WeakMap<WebSocket, string>()
  private wsAlive = new WeakMap<WebSocket, boolean>()

  // Relay discovery registry (Nostr-style)
  private knownRelays = new Map<string, RelayInfo>()
  ...
  constructor(opts?: TinkerRelayOptions) {
    ...
    if (opts?.peerRelays) process.env.PEER_RELAYS = opts.peerRelays.join(',')
  }
  ...
  private log(...args: unknown[]): void {
    if (this.verbose) console.log('[relay]', ...args)
  }

  private jsonResp(res: http.ServerResponse, status: number, body: unknown): void {
    res.writeHead(status, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify(body))
  }

  private announceRelay(info: RelayInfo): void {
    this.knownRelays.set(info.address, info)
    this.log(`🔗 relay_announce: ${info.address} (${info.nodeId.slice(0, 8)}, peers=${info.peerCount})`)
  }

  private getKnownRelays(): RelayInfo[] {
    return [...this.knownRelays.values()]
  }

  /** Auto-create a #cap:<capability> channel if it doesn't exist */
  private ensureCapabilityChannel(capability: string, description?: string): CapabilityChannel {
    const existing = getCapabilityChannel(capability)
    if (existing) return existing
    const now = Date.now()
    const channel: CapabilityChannel = {
  ...
  async start(): Promise<void> {
    ...
    if (persisted.length > 0) this.log(`♻️  loaded ${persisted.length} agents from DB`)
  ...
  async stop(): Promise<void> {
    ...
    if (this.reputationInterval) { clearInterval(this.reputationInterval); this.reputationInterval = null }
    if (this.vitalityInterval)   { clearInterval(this.vitalityInterval);   this.vitalityInterval   = null }
  ...
  private startIntervals(): void {
    ...
    this.heartbeatInterval = setInterval(() => {
      this.wss?.clients.forEach((ws) => {
        if (this.wsAlive.get(ws) === false) { ws.terminate(); return }
    ...
    this.reputationInterval = setInterval(() => {
      try {
        ...
        const activeNodes = db.prepare(
          `SELECT DISTINCT from_id FROM messages WHERE timestamp > ?`
        ).all(fiveMinAgo) as { from_id: string }[]
        for (const { from_id } of activeNodes) {
          try { recalculateReputation(from_id) } catch { /* ignore per-node errors */ }
        }
        if (activeNodes.length > 0) {
          this.log(`🔄 recalculated reputation for ${activeNodes.length} active node(s)`)
        }
      } catch (err) {
        this.log('⚠️ reputation job error:', (err as Error).message)
      }
    }, 60_000)
      ...
      try {
        ...
        for (const agent of staleAgents) {
          ...
          broadcastGlobal(JSON.stringify({
            type: 'agent_status', nodeId: agent.nodeId, status: 'stale' as const,
          } satisfies SignalingMsg))
        ...
        if (expiredBans > 0) this.log(`🔓 cleaned ${expiredBans} expired ban(s)`)

        // ⑥ Expire proposals
        const expiredProps = dbExpireProposals()
        if (expiredProps > 0) this.log(`🗳️ expired ${expiredProps} proposal(s)`)
      } catch (err) {
        this.log('⚠️ vitality job error:', (err as Error).message)
      }
    }, 60_000)
  }
  ...
  private handleRequest(req: http.IncomingMessage, res: http.ServerResponse): void {
    ...
    if (req.method === 'POST' && url.pathname === '/relays/announce') {
      ...
      req.on('end', () => {
        try {
          const info = JSON.parse(body) as RelayInfo
    ...
    if (req.method === 'GET' && agentMatch) {
      ...
      if (!info) { this.jsonResp(res, 404, { ok: false, message: 'agent not found' }); return }
    ...
    if (req.method === 'POST' && url.pathname === '/agents/register') {
      ...
      req.on('end', () => {
        try {
          const payload = JSON.parse(body) as RegisterRequest
          ...
          const resp: RegisterResponse = { ok: true, nodeId: payload.nodeId, message: 'Agent registered' }
    ...
    if (req.method === 'GET' && profilesFacetMatch) {
      ...
      if (profiles.length === 0) { this.jsonResp(res, 404, { ok: false, message: 'profile not found' }); return }
    ...
    if (req.method === 'POST' && url.pathname === '/profiles') {
      ...
      req.on('end', () => {
        try {
          const payload = JSON.parse(body) as AgentProfile
    ...
    if (req.method === 'GET' && reputationMatch) {
      ...
      if (!score) { this.jsonResp(res, 404, { ok: false, message: 'no reputation data for this node' }); return }
    ...
    if (req.method === 'GET' && channelMatch) {
      ...
      if (!channel) { this.jsonResp(res, 404, { ok: false, message: 'channel not found' }); return }
    ...
    if (req.method === 'POST' && url.pathname === '/channels') {
      ...
      req.on('end', () => {
        try {
          const payload = JSON.parse(body) as { capability: string; description?: string }
    ...
    if (req.method === 'GET' && url.pathname === '/intents') {
      ...
      if (capability) intents = intents.filter(i => i.capability === capability)
    ...
    if (req.method === 'GET' && intentMatch) {
      ...
      if (!intent) { this.jsonResp(res, 404, { ok: false, message: 'intent not found' }); return }
      ...
      req.on('end', () => {
        try {
          const payload = JSON.parse(body) as {
            fromNodeId: string; capability: string; description: string
            constraints?: Record<string, unknown>; ttlMs?: number
          }
          ...
          const intent: Omit<Intent, 'status' | 'matchedBy' | 'matchedAt'> = {
            intentId,
    ...
    if (req.method === 'POST' && url.pathname === '/bans') {
      ...
      req.on('end', () => {
        try {
          const { nodeId, reason, expiresAt } = JSON.parse(raw) as { nodeId: string; reason: string; expiresAt?: number }
          if (!nodeId || !reason) { this.jsonResp(res, 400, { ok: false, message: 'nodeId and reason required' }); return }
          const ban = dbBanAgent(nodeId, 'relay', reason, expiresAt)
          broadcastGlobal(JSON.stringify({ type: 'ban', nodeId, reason, expiresAt } satisfies SignalingMsg))
          gossipBroadcast(JSON.stringify({ type: 'gossip_ban', ban, gossiped: true } satisfies SignalingMsg))
          this.log(`🚫 banned: ${nodeId.slice(0, 8)} — ${reason}`)
          this.jsonResp(res, 200, { ok: true, ban })
        } catch { this.jsonResp(res, 400, { ok: false, message: 'invalid JSON' }) }
      })
      return
    }
    ...
    if (req.method === 'DELETE' && banMatch) {
      ...
      dbUnbanAgent(nodeId)
      broadcastGlobal(JSON.stringify({ type: 'unban', nodeId } satisfies SignalingMsg))
    ...
    if (req.method === 'GET' && rulesMatch) {
      ...
      if (!rules) { this.jsonResp(res, 404, { ok: false, message: 'no rules for room' }); return }
    ...
    if (req.method === 'POST' && url.pathname === '/rules') {
      ...
      req.on('end', () => {
        try {
          const rules = JSON.parse(raw) as RoomRules
          if (!rules.roomId || !rules.creator) { this.jsonResp(res, 400, { ok: false, message: 'roomId and creator required' }); return }
          rules.updatedAt = Date.now()
          dbSetRoomRules(rules)
          broadcast(rules.roomId, JSON.stringify({ type: 'room_rules_update', rules } satisfies SignalingMsg))
    ...
    if (req.method === 'POST' && url.pathname === '/proposals') {
      ...
      req.on('end', () => {
        try {
          const { roomId, proposedBy, changes, ttlMs } = JSON.parse(raw) as {
            roomId: string; proposedBy: string; changes: Record<string, unknown>; ttlMs?: number
          }
          if (!roomId || !proposedBy || !changes) {
            this.jsonResp(res, 400, { ok: false, message: 'roomId, proposedBy, changes required' }); return
          }
          const rules = dbGetRoomRules(roomId)
          ...
          const proposal = dbCreateProposal(
            proposalId, roomId, proposedBy,
            changes as Partial<Omit<RoomRules, 'roomId' | 'creator' | 'updatedAt'>>,
            ttlMs
          )
          broadcast(roomId, JSON.stringify({ type: 'rule_proposal', proposal } satisfies SignalingMsg))
    ...
    if (req.method === 'POST' && voteMatch) {
      ...
      req.on('end', () => {
        try {
          ...
          const { voter, vote } = JSON.parse(raw) as { voter: string; vote: 'for' | 'against' }
          if (!voter || !vote) { this.jsonResp(res, 400, { ok: false, message: 'voter and vote required' }); return }

          const proposal = dbGetProposal(proposalId)
          if (!proposal) { this.jsonResp(res, 404, { ok: false, message: 'proposal not found' }); return }
          ...
          if (!updated) { this.jsonResp(res, 404, { ok: false, message: 'proposal not found or closed' }); return }

          broadcast(proposal.roomId, JSON.stringify({ type: 'rule_vote', proposalId, roomId: proposal.roomId, vote, voter } satisfies SignalingMsg))

          const result = dbEvaluateProposal(proposalId)
          if (result === 'passed') {
            const newRules = dbGetRoomRules(proposal.roomId)
            if (newRules) {
              broadcast(proposal.roomId, JSON.stringify({ type: 'room_rules_update', rules: newRules } satisfies SignalingMsg))
  ...
  private async handleWsMessage(ws: WebSocket, rawStr: string): Promise<void> {
    let msg: SignalingMsg
    try {
      msg = JSON.parse(rawStr) as SignalingMsg
    } catch {
      ws.send(JSON.stringify({ type: 'error', code: 'PARSE_ERROR', message: 'invalid JSON' } satisfies SignalingMsg))
      return
    }
    ...
    switch (msg.type) {
      ...
      case 'hello': {
        ...
        if (dbIsBanned(nodeId)) {
          ws.send(JSON.stringify({ type: 'error', code: 'BANNED', message: 'you are banned from this relay' } satisfies SignalingMsg))
        ...
        broadcastGlobal(JSON.stringify({ type: 'agent_joined', agent: agentInfo } satisfies SignalingMsg), nodeId)
        break
      }
      ...
      case 'join': {
        if (this.authRequired) {
          ...
          if (!result.ok) {
            ws.send(JSON.stringify({ type: 'error', code: 'AUTH_FAILED', message: result.reason ?? 'auth failed' } satisfies SignalingMsg))
        ...
        ws.send(JSON.stringify({ type: 'members', roomId: msg.roomId, members: getRoomMembers(msg.roomId) } satisfies SignalingMsg))

        const sysMsg: TinkerMessage = {
        ...
        broadcast(msg.roomId, JSON.stringify({ type: 'message', payload: sysMsg } satisfies SignalingMsg), msg.nodeId)
        ...
        broadcastGlobal(JSON.stringify({ type: 'agent_joined', agent: agentInfo } satisfies SignalingMsg), msg.nodeId)
        break
      }
      ...
      case 'message': {
        ...
        if (this.msgAuth) {
          ...
          if (!pubKey) {
            ws.send(JSON.stringify({ type: 'error', code: 'UNKNOWN_SENDER', message: 'sender not in room' } satisfies SignalingMsg))
            return
          }
          ...
          if (!result.ok) {
            ws.send(JSON.stringify({ type: 'error', code: 'MSG_AUTH_FAILED', message: result.reason ?? 'auth failed' } satisfies SignalingMsg))
            return
          }
        }
        ...
        // v0.9: access policy enforcement
        {
          ...
          if (!access.allowed) {
            ws.send(JSON.stringify({ type: 'error', code: 'ACCESS_DENIED', message: access.reason ?? 'access denied' } satisfies SignalingMsg))
          ...
          if (payload.room && !dbCheckRateLimit(payload.from, payload.room)) {
            ws.send(JSON.stringify({ type: 'error', code: 'RATE_LIMITED', message: 'rate limit exceeded for this room' } satisfies SignalingMsg))
            return
          }
          if (payload.room) {
            const rules = dbGetRoomRules(payload.room)
            if (rules?.maxMessageLength && payload.content.length > rules.maxMessageLength) {
              ws.send(JSON.stringify({ type: 'error', code: 'MSG_TOO_LONG', message: `message exceeds max length ${rules.maxMessageLength}` } satisfies SignalingMsg))
              return
            }
            if (rules?.allowedMessageTypes && !rules.allowedMessageTypes.includes(payload.type)) {
              ws.send(JSON.stringify({ type: 'error', code: 'MSG_TYPE_NOT_ALLOWED', message: `message type '${payload.type}' not allowed in this room` } satisfies SignalingMsg))
              return
            }
          }
        }
        ...
        if (payload.type !== 'system') {
          ...
          if (payload.to) recordEdge(payload.from, payload.to)
          else recordEdge(payload.from, `room:${payload.room}`)
          if (payload.room) {
            try { dbTrackRoomActivity(payload.room, payload.from) } catch { /* ignore */ }
          }
        }

        if (!(msg as unknown as { gossiped?: boolean }).gossiped) {
        ...
        if (payload.to) {
          if (!payload.room) {
            ws.send(JSON.stringify({ type: 'error', code: 'MISSING_ROOM', message: 'room required for room DM; use dm type for roomless DM' } satisfies SignalingMsg))
            return
          }
          const delivered = sendToNode(payload.room, payload.to, serialized)
          if (!delivered) {
            ws.send(JSON.stringify({ type: 'error', code: 'DELIVERY_FAILED', message: `target ${payload.to.slice(0, 8)} not in room` } satisfies SignalingMsg))
          }
          this.log(`💬 room-DM: ${payload.from.slice(0, 8)} → ${payload.to.slice(0, 8)} [${payload.room}]`)
        } else {
          if (!payload.room) {
            ws.send(JSON.stringify({ type: 'error', code: 'MISSING_ROOM', message: 'room required for broadcast' } satisfies SignalingMsg))
            return
          }
        ...
        ws.send(JSON.stringify({ type: 'ack', messageId: payload.id, from: 'relay' } satisfies SignalingMsg))
        break
      }
      ...
      case 'dm': {
        ...
        if (!payload.to) {
          ws.send(JSON.stringify({ type: 'error', code: 'MISSING_TO', message: 'dm requires to field' } satisfies SignalingMsg))
          return
        }
        ...
        if (!delivered) {
          ws.send(JSON.stringify({ type: 'error', code: 'DELIVERY_FAILED', message: `target ${payload.to.slice(0, 8)} is offline or not connected` } satisfies SignalingMsg))
      ...
      // ── gossip_message ─────────────────────────────────────────────────────
      case 'gossip_message': {
        const payload = msg.payload as TinkerMessage
        try { persistMessage(payload) } catch { /* duplicate, ignore */ }
        if (payload.room) {
      ...
      // ── ping ───────────────────────────────────────────────────────────────
      case 'ping': {
        ws.send(JSON.stringify({ type: 'pong' } satisfies SignalingMsg))
        break
      }
      ...
      case 'profile_update': {
        ...
        if (!senderNodeId || senderNodeId !== profile.nodeId) {
          ws.send(JSON.stringify({ type: 'error', code: 'AUTH_FAILED', message: 'can only update your own profile' } satisfies SignalingMsg))
          return
        }
        upsertProfile({
        ...
        if (profile.declared.capabilities) {
          for (const cap of profile.declared.capabilities) {
            ...
            } satisfies SignalingMsg), senderNodeId)
      ...
      case 'profile_request': {
        ...
        ws.send(JSON.stringify({ type: 'profile_response', profiles } satisfies SignalingMsg))
        break
      }
      ...
      case 'endorse': {
        const senderNodeId = this.wsToNodeId.get(ws)
        if (!senderNodeId) {
          ws.send(JSON.stringify({ type: 'error', code: 'NOT_CONNECTED', message: 'must send hello first' } satisfies SignalingMsg))
          return
        }
      ...
      case 'intent_match': {
        ...
        if (!intent) {
          ws.send(JSON.stringify({ type: 'error', code: 'NOT_FOUND', message: 'intent not found' } satisfies SignalingMsg))
          return
        }
        if (intent.fromNodeId !== senderNodeId) {
          ws.send(JSON.stringify({ type: 'error', code: 'AUTH_FAILED', message: 'only intent creator can match' } satisfies SignalingMsg))
          return
        }
        if (intent.status !== 'open') {
          ws.send(JSON.stringify({ type: 'error', code: 'INVALID_STATE', message: `intent status is ${intent.status}, not open` } satisfies SignalingMsg))
          return
        }
        const matched = dbMatchIntent(msg.intentId, msg.matchedNodeId)
        if (matched) {
          broadcastGlobal(JSON.stringify({ type: 'intent_matched', intent: matched } satisfies SignalingMsg))
      ...
      case 'gossip_ban': {
        ...
        if (ban && ban.nodeId) {
          ...
          broadcastGlobal(JSON.stringify({ type: 'ban', nodeId: ban.nodeId, reason: ban.reason, expiresAt: ban.expiresAt } satisfies SignalingMsg))
      ...
      case 'rule_vote': {
        ...
        if (proposalId && roomId && vote && voter) {
          ...
          const updated = dbVoteOnProposal(proposalId, voter, vote, weight)
          if (updated) {
            broadcast(roomId, JSON.stringify({ type: 'rule_vote', proposalId, roomId, vote, voter } satisfies SignalingMsg), voter)
            const result = dbEvaluateProposal(proposalId)
            if (result === 'passed') {
              const newRules = dbGetRoomRules(roomId)
              if (newRules) {
                broadcast(roomId, JSON.stringify({ type: 'room_rules_update', rules: newRules } satisfies SignalingMsg))
  ...
  private handleWsClose(ws: WebSocket): void {
    ...
    if (nodeId) {
      ...
      if (!this.isShuttingDown) {
        try { dbSetOnline(nodeId, false) } catch { /* ignore */ }
        try { dbSetAgentStatus(nodeId, 'offline') } catch { /* ignore */ }
        try { recalculateReputation(nodeId) } catch { /* ignore */ }
        try {
          broadcastGlobal(JSON.stringify({ type: 'agent_status', nodeId, status: 'offline' } satisfies SignalingMsg))
        } catch { /* ignore */ }
        this.log(`👋 disconnected: ${nodeId.slice(0, 8)}`)
        broadcastGlobal(JSON.stringify({ type: 'agent_left', nodeId } satisfies SignalingMsg))
      }
    }
  }
}
